...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="index.php?p=gerretorno">Gerenciar retorno</a></li>
            ...
            <div class="box box-success">
                <div class="box-header with-border">
                   ...
                   if ($id == NULL){
                    ...
                    $soma1 = mysqli_query($mysqli, "SELECT SUM(VALORPAGO) AS PAGO, SUM(DESPCOBRANCA) AS COBRANCA, SUM(DESPPROTESTO) AS PROTESTO FROM RETORNO WHERE RETORNOARQUVO = {$id}");
                    $soma = mysqli_fetch_object($soma1);
                    $vlpg = mysqli_query($mysqli, "SELECT SUM(VALORPAGO) AS PAGO FROM RETORNO WHERE RETORNOARQUVO = {$id} AND OCORRENCIA <> 28");
                    ...
                    $sql_code = "SELECT USUARIO.NOME, RETORNO.ID, RETORNO.DATA, RETORNO.HORA, RETORNO.RESPONSAVEL, RETORNO.USUARIO, RETORNO.BOLETO, RETORNO.OCORRENCIA, RETORNO.DATAOCORRENCIA, RETORNO.DESPCOBRANCA, RETORNO.DESPPROTESTO, RETORNO.ABATIMENTO, RETORNO.DESCONTO, RETORNO.VALORPAGO, RETORNO.JUROSMORA, RETORNO.MULTA, RETORNO.NOSSONUMERO, RETORNO.RETORNOARQUVO FROM RETORNO INNER JOIN MOTIVOOCORENCIA ON RETORNO.ID = MOTIVOOCORENCIA.RETORNO INNER JOIN USUARIO ON USUARIO.ID = RETORNO.USUARIO INNER JOIN OCORRENCIA ON OCORRENCIA.COD = RETORNO.OCORRENCIA "
                            . "WHERE RETORNO.RETORNOARQUVO = {$id}  ORDER BY USUARIO.NOME LIMIT {$limit}, {$itens_por_pagina}";
                    $execute = $mysqli->query($sql_code) or die($mysqli->error);
                    $produto = $execute->fetch_assoc();
                    $num = $execute->num_rows;

                    // pega a quantidade total de objetos no banco de dados
                    $num_total = $mysqli->query("SELECT RETORNO.ID FROM RETORNO WHERE RETORNO.RETORNOARQUVO = {$id} ")->num_rows;
                            ...
                            <div class="col-md-12">
                                <ul class="pagination">
                                    ...
                                    if ($pagina == 0) {
                                        ...
                                        ?>
                                        <li><a href="index.php?p=verretorno&pg=<?= $pagina - 1 ?>&id=<?= $id ?>">&lt; Anterior</a></li>
                                        <?php
                                    }
                                    for ($i = 0; $i < $num_paginas; $i++) {
                                        ...
                                        <li>
                                        <li <?php echo $estilo; ?>  ><a href="index.php?p=verretorno&pg=<?php echo $i; ?>&id=<?= $id ?>"><?php echo $i + 1; ?></a></li>
                                    <?php } ?>
                                    <li>
                                        <?php
                                        ...
                                        ?>                                        
                                        <li class="next"><a href="index.php?p=verretorno&pg=<?= $pagina + 1 ?>&id=<?= $id ?>" rel="next">Próximo &gt;</a></li>
                                        <?php
                                    } ?>
                                </ul><!-- /.pagination -->
                            </div>
